...
import { checkModules, MissingBackendModules } from "./backend-dependencies/openmrs-backend-dependencies";
...
const PopupHandler = connect("isOpen")(({ isOpen }) => {
    ...
    const [modulesWithMissingBackendModules, setModulesWithMissingBackendModules] = useState<
        Array<MissingBackendModules>
    >([]);
    const [modulesWithWrongBackendModulesVersion, setModulesWithWrongBackendModulesVersion] = useState<
        Array<MissingBackendModules>
    >([]);
    ...
    useEffect(() => {
        checkModules().then(({ modulesWithMissingBackendModules, modulesWithWrongBackendModulesVersion }) => {
            setModulesWithMissingBackendModules(modulesWithMissingBackendModules);
            setModulesWithWrongBackendModulesVersion(modulesWithWrongBackendModulesVersion);
        });
    }, []);

    // displaying toast if modules are missing
    useEffect(() => {
        if (!(modulesWithMissingBackendModules.length > 0 || modulesWithWrongBackendModulesVersion.length > 0)) return;
        ...
        showToast({
            description: <span>Found modules with unresolved backend dependencies.</span>,
            action: <NotificationActionButton onClick={showModuleDiagnostics}>View</NotificationActionButton>,
            kind: "error",
        });
    ...
    return (
        <>
            <button
                onClick={togglePopup}
                className={`${styles.popupTriggerButton} ${hasAlert ? styles.triggerButtonAlert : ""}`}
            />
            {isOpen ? (
                <Popup
                    close={togglePopup}
