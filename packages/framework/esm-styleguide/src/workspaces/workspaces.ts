...
export interface DefaultWorkspaceProps {
  ...
  /**
   * Sets the onCloseCallback to be invoked when the workspace is to be closed.
   *
   * This function allows you to specify actions that should be performed before the workspace is closed.
   *
   * @param callback - The function to be called when the workspace is being closed.
   */
...
>(name: string, additionalProps?: Omit<T, keyof DefaultWorkspaceProps> & { workspaceTitle?: string }) {
  ...
  const newWorkspace = {
    ...
    setOnCloseCallback(callback: () => void) {
      newWorkspace.onCloseCallback = callback;
    },
