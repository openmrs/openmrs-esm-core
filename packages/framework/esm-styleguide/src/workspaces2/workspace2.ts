import { getGroupByWindowName, getOpenedWindowIndexByWorkspace, getWindowByWorkspaceName, OpenedWindow, OpenedWorkspace, workspace2Store, WorkspaceStoreState2 } from "@openmrs/esm-extensions";
...
>(
  ...
  if(!groupDef) {
    throw new Error(`Unable to launch workspace ${workspaceName}. Workspace window ${windowDef.name} is not registered to a workspace group`);
  }
  ...
  if(openedGroup && (openedGroup.groupName !== groupDef.name || !arePropsCompatible(openedGroup.props, groupProps))) {
    ...
    if(arePropsCompatible(openedWindow.props, windowProps)) {
      ...
      if(openedWorkspace && arePropsCompatible(openedWorkspace.props, workspaceProps)) {
        ...
        const okToCloseWorkspaces = await promptForClosingWorkspaces({reason: 'CLOSE_WORKSPACE', explicit: false, windowName, workspaceName});
        if(okToCloseWorkspaces) {
          workspace2Store.setState({
            ...storeState,
            openedGroup: {
      ...
      const okToCloseWorkspaces = await promptForClosingWorkspaces({reason: 'CLOSE_WINDOW', explicit: false, windowName});
      if(okToCloseWorkspaces) {
...
export function promptForClosingWorkspaces(promptReason: PromptReason): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    const dispose = showModal('workspace2-close-prompt', {
      ...
      affectedWorkspaceTitles: affectedWorkspaces.map(workspace => workspaceTitleByWorkspaceName[workspace.workspaceName]),
    })
  });
} 
    ...
    return {
      openedWindows: state.openedWindows.map((w, i) => {
        if(i == openedWindowIndex) {
          return {...w, openedWorkspaces: [...w.openedWorkspaces, newOpenedWorkspace(childWorkspaceName, childWorkspaceProps)]};
        } else {
          return w;
        }
      })
