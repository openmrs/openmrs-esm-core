...
import styles from './patient-banner-info.scss';
...
export function PatientBannerPatientInfo({ patientUuid, name, gender, identifiers, birthDate, deathDate }: PatientBannerPatientInfoProps) {
  const { excludePatientIdentifierCodeTypes } = useConfig();

  const filteredIdentifiers = identifiers.filter(
    (identifier) => !excludePatientIdentifierCodeTypes?.uuids.includes(identifier.typeUuid),
  ) ?? [];
